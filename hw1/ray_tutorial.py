...
#       whereas `remote_function` immediately returns an object ID (a future) and
#       then creates a task that will be executed on a worker process. The result can be obtained with `ray.get`.
#     ```python
#     >>> regular_function(0)
#     1
#
...
# Usually this is not done and Ray computes the number of CPUs using `psutil.cpu_count()`.
...
# execute these four tasks in parallel by calling `slow_function.remote()`. Then obtain the results by calling
# `ray.get` on a list of the resulting object IDs.
